...
//define node struct, build tree given N points, point search, range search (find all points within specified bounding rectangle
...
struct Stopq {
	...
	Stopq(double lat, double lon, int id = 0, const std::string& code = "", const std::string& name = "") : lat(lat), lon(lon), id(id), code(code), name(name) {} 
    Stopq() = default;
    ...
    bool insert(const Stopq& stop) { 
        ...
        // the last node to hold the data
         ...
         return false;
          ...
          std::vector<Stopq> queryRange(const AABB& range) const { //stop should be used here in addition to AABB
