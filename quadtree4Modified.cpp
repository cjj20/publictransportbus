...
struct AABB {
    ...
    bool containsPoint(const Stopq& stop) const {
	   ...
	   if (!std::isnan(stop.lon) && !std::isnan(stop.lat)) {
        ...
        if(stop.lon > (centerX1 + halfDimensionX)  ||  stop.lon < (centerX1 - halfDimensionX)) //should be minus here to return true
            return false;
        ...
        if  (stop.lat > (centerY1 + halfDimensionY) || stop.lat < (centerY1 - halfDimensionY))
            return false;
		...
		*/
        ...
        if(std::stod(stop.gtfsstop1.stop_lon) > (centerX1 + halfDimensionX)  || std::stod(stop.gtfsstop1.stop_lon) < (centerX1 - halfDimensionX)) 
            return false;
       ...
       // if( std::stod(stop.gtfsstop1.stop_lon) < (centerX1 - halfDimensionX))
        ...
        if  (std::stod(stop.gtfsstop1.stop_lat) > (centerY1 + halfDimensionY) || std::stod(stop.gtfsstop1.stop_lat) < (centerY1 - halfDimensionY))
            return false;
		...
		*/
        ...
        if(stop.osmcoordntaggs.loni > (centerX1 + halfDimensionX) || stop.osmcoordntaggs.loni < (centerX1 - halfDimensionX)) 
            return false;
       ...
       // if(stop.osmcoordntaggs.loni < (centerX1 - halfDimensionX))
        ...
        if  (stop.osmcoordntaggs.lati > (centerY1 + halfDimensionY) || stop.osmcoordntaggs.lati < (centerY1 - halfDimensionY))
            return false;
    ...
    bool insert(const Stopq& stop) { 
        ...
        std::vector<Stopq> searchByProximity(double lat, double lon, double radius) {
		   ...
		   double earth_radius = 6371000.0;
			...
			if (!std::isnan(stopqq.osmcoordntaggs.lati) && !std::isnan(stopqq.osmcoordntaggs.loni)) {
                double olat = stopqq.osmcoordntaggs.lati;
                double olon = stopqq.osmcoordntaggs.loni;
				auto osmres = searchByProxim(olat, olon, radius);
				//saving vector into vector
				stopqresults.insert(stopqresults.begin(), osmres.begin(), osmres.end());
			...
			if (!stopqq.gtfsstop1.stop_lat.empty() && !stopqq.gtfsstop1.stop_lon.empty()) {
				...
				stopqresults.insert(stopqresults.begin(), gtfsres.begin(), gtfsres.end());
			...
			if (!std::isnan(stopqq.lat) && !std::isnan(stopqq.lon)){
				...
				stopqresults.insert(stopqresults.begin(), otherres.begin(), otherres.end());
		...
		*/
		...
		static double haversDistance(double lat1, double lon1, double lat2, double lon2) {
            ...
            return 2.0 * earthRadiusM * asin(sqrt(u * u + cos(latRad1) * cos(latRad2) * v * v)); //distance in metres
		...
		static double getOsmCoordinates(double targetLat, double targetLon, const Stopq& stopqq) {
			
			double lat;
			double lon;
			
            if (!std::isnan(stopqq.osmcoordntaggs.lati) && !std::isnan(stopqq.osmcoordntaggs.loni)) {
            lat = (stopqq.osmcoordntaggs.lati < 1e-6) ? stopqq.osmcoordntaggs.lati : roundToDecimalPlaces(stopqq.osmcoordntaggs.lati, 6);
            lon = (stopqq.osmcoordntaggs.loni < 1e-6) ? stopqq.osmcoordntaggs.loni : roundToDecimalPlaces(stopqq.osmcoordntaggs.loni, 6);
		...
		static double getGtfsCoordinates(double targetLat, double targetLon, const Stopq& stopqq) {
			double lat;
			double lon;
			...
			if (!stopqq.gtfsstop1.stop_lat.empty() && !stopqq.gtfsstop1.stop_lon.empty()) {
				try {
					...
					lon = (tempLon < 1e-6) ? tempLon : roundToDecimalPlaces(tempLon, 6);
						...
						// Handle conversion error by setting lat and lon to 0.0 (already initialized)
						lat = 0.0;
						lon = 0.0;
					}
					//haversDistance(targetLat, targetLon, lat, lon);
		...
		static double haversDist2(double targetLat, double targetLon, const Stopq& stopqq) {
			...
			if ((!stopqq.gtfsstop1.stop_lat.empty() && !stopqq.gtfsstop1.stop_lon.empty()) || (!std::isnan(stopqq.osmcoordntaggs.lati) && !std::isnan(stopqq.osmcoordntaggs.loni)) || (!std::isnan(stopqq.lat) && !std::isnan(stopqq.lon))){
			...
			/*
               ...
               // lon = roundToDecimalPlaces(stopqq.lon, 6);
			     lat = (stopqq.lat  < 1e-6) ? stopqq.lat : roundToDecimalPlaces(stopqq.lat, 6);
                 lon = (stopqq.lon  < 1e-6) ? stopqq.lon : roundToDecimalPlaces(stopqq.lon, 6);
		...
		static double haversDist(double targetLat, double targetLon, const Stopq& stopqq) {
            ...
            if (!stopqq.gtfsstop1.stop_lat.empty() && !stopqq.gtfsstop1.stop_lon.empty()) {
				try{
               // lat = roundToDecimalPlaces(std::stod(stopqq.gtfsstop1.stop_lat), 6);
               // lon = roundToDecimalPlaces(std::stod(stopqq.gtfsstop1.stop_lon), 6);
			      double tempLat = std::stod(stopqq.gtfsstop1.stop_lat);
                  double tempLon = std::stod(stopqq.gtfsstop1.stop_lon);
			      lat = (tempLat  < 1e-6) ? tempLat : roundToDecimalPlaces(tempLat, 6);
                  lon = (tempLon  < 1e-6) ? tempLon : roundToDecimalPlaces(tempLon, 6);
		...
		double calculateDistance(double targetLat, double targetLon, const Stopq& stopqq )  {
            return haversDist(targetLat, targetLon, stopqq);
		
        }
		
		double calculateDistance2(double targetLat, double targetLon, const Stopq& stopqq )  {
            return haversDist2(targetLat, targetLon, stopqq);
		
        }
		
		double calculateDistance3(double targetLat, double targetLon, const Stopq& stopqq )  {
            return getGtfsCoordinates(targetLat, targetLon, stopqq);
		
        }
		
		double calculateDistance4(double targetLat, double targetLon, const Stopq& stopqq )  {
		...
		///std::vector<Stopq> searchByProxim2(const std::vector<Stopq>& points, double lat, double lon, double radius) {
		std::vector<Stopq> searchByProxim2(double lat, double lon, double radius) {
            ...
            std::cout << "Searching for points within radius: " << radius << " from (" << lat << ", " << lon << ")" << std::endl;
        ...
        for (const auto& point : candidates) {
			...
			//for(const auto& allpoints : candidates2) {
				...
				std::cout << "Distance from (" << lat << ", " << lon << ") to point (" 
                  << allpoints.gtfsstop1.stop_lat << ", " << allpoints.gtfsstop1.stop_lon << "): " << distanceFrom << std::endl;
				std::cout << "Distance from (" << lat << ", " << lon << ") to point (" 
                  << allpoints.osmcoordntaggs.lati << ", " << allpoints.osmcoordntaggs.loni << "): " << distanceFrom << std::endl;
				*/
			...
			std::cout << "Total number of points contained within given radius: " << results2.size() << std::endl;
			return results2;
		}
		
		
		 AABB createBoundingBox(double lat, double lon, double radius) const {
            double halfWidth = radius / 111320; // Approximate conversion from meters to degrees
            double halfHeight = radius / 110540; // Approximate conversion from meters to degrees
            return AABB(lon - halfWidth, lat - halfHeight, lon + halfWidth, lat + halfHeight);
        }
...
*/	  
	...
	*/
	...
	for (const auto& point : quadTree.points) {
       quadTree.insert(point);
	   outputFile2 << "Stop Name: " << point.name 
                  << ", ID: " << point.id 
                  << ", Code: " << point.code
                  << ", Latitude: " << point.lat			  
                  << ", Longitude: " << point.lon 
                  ...
                  << ", OSM Coord Lon: " << point.osmcoordntaggs.loni
    ...
    Stop stop11(std::string("2"), std::string("A Ave & Chandler"), std::string("45.420609"), std::string("-122.675671"));
